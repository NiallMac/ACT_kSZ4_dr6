...
def get_CLKK_stuff(data_dir, rdn0_file, binner, w1=None, w4=None, nsim_meanfield=None, meanfield_dir=None, use_mcn0=False,
    ...
    if check_auto:
        try:
            isfin = np.isfinite(CL_KK_raw_from_Ks[binner.lmin:binner.lmax])
            assert np.allclose(CL_KK_raw_from_Ks[binner.lmin:binner.lmax][isfin], outputs["CL_KK_raw"][binner.lmin:binner.lmax][isfin], rtol=1.e-3)
    ...
    if correct_mf_auto:
        """
        If we calculate the mean-field with an <SS><SS> type approach, then we
        end up actually calculating meanfield + MCN0/nsim_meanfield 
        so we need to 
        """
...
def plot_clKK(ax, outputs_dir, binner, mask=None, color=None, label=None, 
             ...
             transfer_func=1., use_alt_norm=False, use_r24_norm=False, w1=None, w4=None):
